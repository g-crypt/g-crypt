...
func TestDecryptAES256WithInvalidPaddingBytes(t *testing.T) {
    text := "Example text"
    password := "secret-password"
    ...
    // Decodificar o texto cifrado para bytes
    encryptedBytes, err := base64.StdEncoding.DecodeString(encrypted)
    if err != nil {
        t.Errorf("Error decoding base64: %v", err)
    }

    // Verificar se o comprimento de encryptedBytes é suficiente
    if len(encryptedBytes) <= aes.BlockSize {
        t.Errorf("Ciphertext length is too short")
        return
    }

    // Pegar o último byte como o padding original
    padding := encryptedBytes[len(encryptedBytes)-1]

    // Corromper o padding, alterando o último byte de padding
    encryptedBytes[len(encryptedBytes)-1] = byte(padding + 1) // Alterar para um valor inválido

    invalidEncrypted := base64.StdEncoding.EncodeToString(encryptedBytes)

    // Tentar descriptografar com padding corrompido
    _, err = DecryptAES256(invalidEncrypted, password)
    
    if err == nil {
        t.Error("Expected error when decrypting with invalid padding bytes, but got none")
    } else if err.Error() != "invalid padding" {
        t.Errorf("Expected 'invalid padding' error, but got: %v", err)
    }
}